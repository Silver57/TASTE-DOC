...
the \texttt{simple} template. The cross-method ranking is preserved on both
alternatives (Table~\ref{tab:prompt_acc}). SimPO is the best trained method
under all three templates; \texttt{icl\_chat} is the best method overall
...
\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{images/recsys_goodreads.png}
...
The methods in this thesis are evaluated by scoring fixed pairwise completions, but a deployed recommender writes its suggestions as natural-language sentences. This sub-study (\S4.5) asks whether the methods still rank the same way when the model must write a recommendation instead of score one. Two metrics are reported in Table~\ref{tab:judge_results}: the rate at which a Claude Sonnet judge prefers each method's sentence head-to-head, and the rate at which the title named in the sentence matches the book the user actually liked in the held-out set.
